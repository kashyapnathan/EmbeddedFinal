...
public class NewEmulator {
	...
	public NewEmulator() {
		...
		transitionU = new TransitionTable(rubbos.getNbOfColumns(), rubbos.getNbOfRows(), null,
				rubbos.useTPCWThinkTime());
		transitionA = new TransitionTable(rubbos.getNbOfColumns(), rubbos.getNbOfRows(), null,
				rubbos.useTPCWThinkTime());
	...
	private static void injectHighLoad(Stats stats, NewEmulator client) {
		int burstSize = 10;
		for (int i = 0; i < burstSize; i++) {
			// Create intensive request sessions
			UserSession burst = createSession(i, client.urlGen, client.rubbos, stats);
			// Configure session for high load
			burst.start();
			list.offer(burst);
		}
		// Wait for 100ms to create bottleneck
		sleep(100);
	}

	private static long getCPUUsage() {
		try {
			String[] cmd = { "/bin/sh", "-c", "top -bn1 | grep \"Cpu(s)\" | awk '{print $2 + $4}'" };
	...
	private static long getMemoryUsage() {
		try {
			String[] cmd = { "/bin/sh", "-c", "free -m | grep Mem | awk '{print $3}'" };
			Process p = Runtime.getRuntime().exec(cmd);
			BufferedReader br = new BufferedReader(new InputStreamReader(p.getInputStream()));
			String line = br.readLine();
			return Long.parseLong(line.trim());
	...
	public static synchronized UserSession createSession(int i, URLGenerator URLGen, RUBBoSProperties RUBBoS,
			Stats statistics) {
		UserSession us = null;
	...
	public static void main(String[] args) {
		...
		Map<String, List<Long>> cpuMetrics = new HashMap<>();
		Map<String, List<Long>> memoryMetrics = new HashMap<>();
		Map<String, List<Long>> responseTimeMetrics = new HashMap<>();
		cpuMetrics.put("node0", new ArrayList<>());
		memoryMetrics.put("node0", new ArrayList<>());
		responseTimeMetrics.put("timeline", new ArrayList<>());
		// END ADD
		...
		if (isMainClient) {
			...
			for (int i = 0; i < client.rubbos.getRemoteClients().size(); i++) {
				try {
					System.out.println("NewEmulator: Starting remote client on "
					...
					System.out.println("&nbsp &nbsp Command is: " + rcmdClient[0] + " " + rcmdClient[1] + " "
							+ rcmdClient[2] + " " + rcmdClient[3] + " " + rcmdClient[4] + "<br>\n");
		...
		for (int i = 0; i < client.rubbos.getRemoteClients().size(); i++)
			System.out.println("<A HREF=\"trace_client" + (i + 1) + ".html\">client" + (i + 1) + " ("
		...
		// ADD BOTTLENECK METHODS
		private static boolean shouldInjectBottleneck(int i) {
			return i % 50 == 0; // Create bottleneck every 50 sessions
		}

		private static final int BURST_SIZE = 5; // Number of extra sessions in burst
		// END ADD
		for (int i = 0; i < wks[0]; i++) {
			us = createSession(count++, client.urlGen, client.rubbos, stats);// create us one by one
			if (shouldInjectBottleneck(i)) {
				// Create burst of sessions
				for (int j = 0; j < BURST_SIZE; j++) {
					UserSession burst = createSession(count++,client.urlGen,client.rubbos,stats);
					burst.start();
					list.offer(burst);
				}
			}
		...
		// sleep(200000);
		// sleep(400000);
		long testDuration = 400000; // 400 seconds
		long startTime = System.currentTimeMillis();
		long nextBottleneck = startTime + 5000; // First bottleneck after 5 seconds

		while (System.currentTimeMillis() - startTime < testDuration) {
			long currentTime = System.currentTimeMillis();
			
			// Create periodic bottlenecks
			if (currentTime >= nextBottleneck) {
				// Inject 100ms bottleneck
				injectHighLoad(stats, client);
				nextBottleneck = currentTime + 5000; // Next bottleneck in 5 seconds
				
				// Collect metrics
				cpuMetrics.get("node0").add(getCPUUsage());
				memoryMetrics.get("node0").add(getMemoryUsage());
			}
		...
		// }
		// Date mark = new Date();
		// buddy.println(mark+" current workload is: "+wks[i]+" list
		// size:"+list.size());
		// sleep(30000);
		// }
		...
		// allStats.display_stats("Overall", TimeManagement.diffTimeInMs(ExpBeginDate,
		// ExpEndDate), false);
		// JB
		...
		if (isMainClient) {
			...
			try {
				...
				buddy.println(
						"web server cmdWeb: " + cmdWeb[0] + " " + cmdWeb[1] + " " + cmdWeb[2] + " " + cmdWeb[3] + " ");
				...
				System.out.println("<A HREF=\"stat_client0.html\">Main client (localhost) statistics</A><br>");
				for (int i = 0; i < client.rubbos.getRemoteClients().size(); i++)
					System.out.println("<A HREF=\"stat_client" + (i + 1) + ".html\">client " + (i + 1) + " ("
							+ client.rubbos.getRemoteClients().get(i) + ") statistics</A><br>");
				System.out.println("<p><br>&nbsp&nbsp&nbsp<A HREF=\"perf.html#node\">Node information</A><br>");
				System.out.println("&nbsp&nbsp&nbsp<A HREF=\"#time\">Test timing information</A><br>");
				System.out.println("&nbsp&nbsp&nbsp<A HREF=\"#up_stat\">Up ramp statistics</A><br>");
		...
		if (servletsFlag) {
			System.out.println(
					"<TD><IMG SRC=\"servlets_server_cpu_user_kernel." + client.rubbos.getGnuPlotTerminal() + "\">");
		}
		System.out.println("<TR><TD><IMG SRC=\"client_cpu_user_kernel." + client.rubbos.getGnuPlotTerminal() + "\">");
		if (cjdbcFlag) {
			System.out.println(
					"<TD><IMG SRC=\"cjdbc_server_cpu_user_kernel." + client.rubbos.getGnuPlotTerminal() + "\">");
		}
		...
		if (servletsFlag) {
			System.out
					.println("<TD><IMG SRC=\"servlets_server_mem_usage." + client.rubbos.getGnuPlotTerminal() + "\">");
		}
		...
		if (servletsFlag) {
			System.out
					.println("<TD><IMG SRC=\"servlets_server_mem_cache." + client.rubbos.getGnuPlotTerminal() + "\">");
		}
		...
		if (servletsFlag) {
			System.out.println(
					"<TD><IMG SRC=\"servlets_server_disk_rw_req." + client.rubbos.getGnuPlotTerminal() + "\">");
		}
		...
		if (servletsFlag) {
			System.out
					.println("<TD><IMG SRC=\"servlets_server_net_rt_byt." + client.rubbos.getGnuPlotTerminal() + "\">");
		}
		...
		if (servletsFlag) {
			System.out.println(
					"<TD><IMG SRC=\"servlets_server_net_rt_pack." + client.rubbos.getGnuPlotTerminal() + "\">");
		}
