...
public class UserSession extends Thread {
	...
	private int lastStoryId = -1; // This is to deal with back because the storyId cannot be retrieved from the
									// current page
	private int lastCategoryId = -1; // This is to deal with back when the categoryId cannot be retrieved from the
										// current page
	private String lastCategoryName = null; // This is to deal with back when the categoryName cannot be retrieved from
											// the current page
	private Stats stats; // Statistics to collect errors, time, ...
	private int debugLevel = 0; // 0 = no debug message, 1 = just error messages, 2 = error messages+HTML pages,
								// 3 = everything!
	private int thisYear; // Current year (2001)
	// private RandomAccessFile dictionary = null; // Dictionary file to use to
	 ...
	 * @param isAuthor   true if this user session is for an author, false for a
	 *                   regular user
	 */
	public UserSession(String threadId, URLGenerator URLGen, RUBBoSProperties RUBBoS, Stats statistics,
			boolean isAuthor, TransitionTable theTransition) {
		super(threadId);
		urlGen = URLGen;
		rubbos = RUBBoS;
		stats = statistics;
		...
		transition = new TransitionTable(rubbos.getNbOfColumns(), rubbos.getNbOfRows(), statistics,
	...
	private String callHTTPServer(URL url) {
		...
		// logMessage = "Yasu0,Thread:" + this.getName() + ",url:" + url + ",st:" +
		// stime;
		...
		while (retry < 5) {
			...
			try {
				...
				// if (debugLevel>0)
				System.err.println("Yasu1: Thread " + this.getName() + ": Unable to open URL " + url + " ("
						+ ioe.getMessage() + ") retry=" + retry + "<br>");
				retry++;
				try {
			...
			try {
				...
				// if (debugLevel>0)
				System.err.println("Yasu2: Thread " + this.getName() + ": Unable to read from URL " + url + " ("
						+ ioe.getMessage() + ") retry=" + retry + "<br>");
				return null;
			}
		...
		try {
			...
			if (debugLevel > 0)
				System.err.println("Thread " + this.getName() + ": Unable to close URL " + url + " (" + ioe.getMessage()
						+ ")<br>");
		}
		...
		while (images.size() > 0) {
			...
			try {
				...
				// BufferedInputStream inImage = new BufferedInputStream(imageURL.openStream(),
				// 4096);
				...
				// if (debugLevel>0)
				System.err.println("Yasu3: Thread " + this.getName() + ": Error while downloading image " + imageURL
						+ " (" + ioe.getMessage() + ") retry=" + retry + ",num=" + num + "<br>");
			}
		...
		// System.err.println(logMessage);
		// }
		// System.out.println("stime " + stime + " Open Connection End " + time1 + "
		// Getting Data " + time5 + " " + time6 + " " + " Get Data End " + time2 + "
		// Close Connection End" + time3 + " etime " + etime);
	 ...
	 * are found, one of them is picked up randomly.
	 *
	 * @return a vector containing first the category identifier (Integer) and then
	...
	/**
	 * Extract post comment parameters (comment_table+storyId+parent) from the last
	 * HTML reply.
	 * If several entries are found, one of them is picked up randomly.
	 *
	 * @return a vector containing first the comment table (String), then the story
	 *         identfier (Integer) and finally the parent identifier (Integer)
	 */
	...
	/**
	 * Extract post comment parameters
	 * (comment_table+storyId+parent+commentId+filter+display) from the last HTML
	 * reply.
	 * If several entries are found, one of them is picked up randomly.
	 *
	 * @return a vector containing first the comment table (String), then the story
	 ...
	 * If several entries are found, one of them is picked up randomly.
	 *
	 * @return First the comment table value (String), then the comment identifier
	 *         (Integer)
	 */
	...
	/**
	 * Extract the story identifier from the review story page for an accept or
	 * reject.
	 * If several entries are found, one of them is picked up randomly.
	 *
	...
	private int extractPageFromHTML() {
		...
		else { // Choose randomly a page (previous or next)
	...
	public String randomWordFromDictionary(boolean withPunctuation) {
		...
		try {
			...
			System.err.println("Unable to read dictionary file '" + rubbos.getStoryDictionary()
					+ "' to generate random word (got exception: " + e.getMessage() + ")<br>");
			return null;
		}
	...
	public URL computeURLFromState(int state) {
		...
		switch (state) {
			...
			{ // We randomly pickup a category from the generated HTML page
				...
				return urlGen.browseStoriesByCategory(categoryId, categoryName, extractPageFromHTML(),
						rubbos.getNbOfStoriesPerPage());
			}
			case 6: // Older stories
			{
				// We have to make hit on past days more successful than hits on 10 years old
				// stories
				int day;
				int month;
				int year;
				int when = rand.nextInt(100);
				if (when > 95) { // Pickup any date
					day = rand.nextInt(31) + 1;
					month = rand.nextInt(12) + 1;
					year = rubbos.getOldestStoryYear() + rand.nextInt(3);
				} else { // Some date in the past week
			...
			case 8: // Post Comment
			{
				...
				if (p == null) {
					if (debugLevel > 0)
						// Yasu:
						// System.out.println("Thread "+this.getName()+": Unable to extract PostComment
						// parameters from last HTML reply.<br>");
						System.err.println("Thread " + this.getName()
								+ ": Unable to extract PostComment parameters from last HTML reply.<br>");
					return null;
				}
			...
			case 9: // Store comment
			{
				...
				return urlGen.storeComment(username, password, extractIntFromHTML("name=storyId value="),
						extractIntFromHTML("name=parent value="),
			...
			case 10: // View comment
			{
				...
				if ((comment_table == null) || (storyId == null) || (commentId == null) || (filter == null)
						|| (display == null))
					return computeURLFromState(transition.backToPreviousState()); // Nothing then go back
				else
					return urlGen.viewComment(commentId.intValue(), filter.intValue(), display.intValue(),
							storyId.intValue(), comment_table);
			}
			...
			case 12: // Moderate log
			{
				...
				return urlGen.storeModerateLog(username, password, extractIntFromHTML("name=commentId value="), rating,
						comment_table);
			}
			...
			case 22: // Accept story
			{
				...
				if (storyId == null) { // No more stories to process go back to home page
					transition.resetToInitialState();
					return urlGen.homePage();
				} else
			...
			default:
				if (debugLevel > 0)
					System.err
							.println("Thread " + this.getName() + ": This state is not supported (" + state + ")<br>");
				return null;
		}
	}
	...
	public void run() {
		...
		try {
			...
			System.err.println("Unable to open dictionary file '" + rubbos.getStoryDictionary() + "' (got exception: "
					+ e.getMessage() + ")<br>");
			return;
		}
		...
		while (!isEndOfSimulation()) {
			...
			if (debugLevel > 2)
				System.out.println(
						"Thread " + this.getName() + ": Starting a new user session for " + username + " ...<br>");
			startSession = System.currentTimeMillis();
			// Start from Home Page
			transition.resetToInitialState();

			// add by Qingyang, the purpose is to avoid every request starting at the same
			// time.
			...
			while (!isEndOfSimulation() && !transition.isEndOfSession() && (nbOfTransitions > 0)) {
				// Compute next step and call HTTP server (also measure time spend in server
				// call)
				...
				if (lastURL == null) {
					// Yasu:
					// System.out.println("Thread "+this.getName()+": Fatal error in computing next:
					// " + next + " URL. Restarting from home page.<br>");
					System.err.println("Thread " + this.getName() + ": Fatal error in computing next: " + next
							+ " URL. Restarting from home page.<br>");
					transition.resetToInitialState();
				...
				if ((lastHTMLReply == null) || (lastHTMLReply.indexOf("ERROR") != -1)) {
					if (debugLevel > 0)
						// Yasu:
						// System.out.println("Thread "+this.getName()+": Error returned from access to
						// "+lastURL+"<br>");
						// System.err.println("Thread "+this.getName()+": Error returned from access to
						// "+lastURL+"<br>");
			...
			if ((transition.isEndOfSession()) || (nbOfTransitions == 0)) {
				if (debugLevel > 2)
					System.out.println(
							"Thread " + this.getName() + ": Session of " + username + " successfully ended<br>");
		...
		try {
			...
			System.err.println("Unable to close dictionary file '" + rubbos.getStoryDictionary() + "' (got exception: "
					+ e.getMessage() + ")<br>");
		}
	...
	private URL singleTypeURLGenerator() {
		...
		// String servletPath =
		// "/rubbos/servlet/edu.rice.rubbos.servlets.ViewStory?storyId=3253";
		// }
		URL url = null;
	...
	private URL doubleTypeURLGenerator() {
		...
		// String light_servletPath =
		// "/rubbos/servlet/edu.rice.rubbos.servlets.StoriesOfTheDay";
		// String light_servletPath =
		// "/rubbos/servlet/edu.rice.rubbos.servlets.ViewComment?commentId=157987&filter=0&display=1&storyId=4445&comment_table=comments";
		// String heavy_servletPath =
		// "/rubbos/servlet/edu.rice.rubbos.servlets.ViewStory?storyId=3253";
	...
	private String servletP() {
		...
		// Create milli-bottleneck patterns similar to your local simulation
		if (shouldCreateBottleneck()) {
			// Generate intensive timeline reads that cause CPU spikes
			int user_id = rand.nextInt(800);
			return "/wrk2-api/home-timeline/read?user_id=" + user_id +
					"&start=1&stop=100"; // Request more data to create load
		}
		// Create bursty patterns with varying intensity
		else if (shouldCreateBurst()) {
			return createBurstyRequests();
		}
	...
	private boolean shouldCreateBottleneck() {
		// Create periodic bottlenecks like in your local tests
		long currentTime = System.currentTimeMillis();
		return (currentTime % 5000) < 100; // 100ms bottleneck every 5 seconds
	}
