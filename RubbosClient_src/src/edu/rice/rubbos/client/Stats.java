...
public class Stats {
    ...
    public synchronized void recordBottleneck(int state, long time, String url) {
        bottleneckTimes.add(time);
        bottleneckUrls.add(url);

        try (FileWriter fw = new FileWriter("bottleneck_log.txt", true)) {
            fw.write(System.currentTimeMillis() + "," + state + "," + time + "," + url + "\n");
        } catch (IOException e) {
            System.err.println("Error logging bottleneck: " + e.getMessage());
        }
    }

    public void writeMetricsToFile(String filename) {
        try (FileWriter fw = new FileWriter(filename)) {
            fw.write("timestamp,response_time,url\n");
            for (int i = 0; i < responseTimes.size(); i++) {
                fw.write(responseTimes.get(i) + "," + bottleneckUrls.get(i) + "\n");
            }
        } catch (IOException e) {
     ...
     * @param exclude0Stat true if you want to exclude the stat with a 0 value from
     *                     the output
     */
    public void display_stats(String title, long sessionTime, boolean exclude0Stat) {
        ...
        System.out.println(
                "<THEAD><TR><TH>State name<TH>% of total<TH>Count<TH>Errors<TH>Minimum Time<TH>Maximum Time<TH>Average Time<TBODY>");
        ...
        for (int i = 0; i < getNbOfStats(); i++) {
            if ((exclude0Stat && count[i] != 0) || (!exclude0Stat)) {
                System.out.print(
                        "<TR><TD><div align=left>" + TransitionTable.getStateName(i) + "</div><TD><div align=right>");
        ...
        if (counts > 0) {
            System.out.print(
                    "<TR><TD><div align=left><B>Total</B></div><TD><div align=right><B>100 %</B></div><TD><div align=right><B>"
                            + counts +
                            "</B></div><TD><div align=right><B>" + errors
            ...
            System.out.println(
                    "<TR><TD><div align=left><B>Average throughput</div></B><TD colspan=6><div align=center><B>"
                            + 1000 * counts / sessionTime + " req/s</B></div>");
            System.out.println("<TR><TD><div align=left>Completed sessions</div><TD colspan=6><div align=left>"
                    + nbSessions + "</div>");
            System.out.println("<TR><TD><div align=left>Total time</div><TD colspan=6><div align=left>"
                    + sessionsTime / 1000L + " seconds</div>");
            System.out.print(
                    "<TR><TD><div align=left><B>Average session time</div></B><TD colspan=6><div align=left><B>");
            if (nbSessions > 0)
    ...
    public void getresult(String reportDir, long sessiontime) {
        try {
            ...
            outfile.println("#" + counts + " " + errors + " " + 1000 * (counts + errors) / sessiontime + " "
                    + time / (counts + errors));
