...
def init(tier):
    ...
    output_file = timeSpan + "_" + tier + "_multiplicity_wl" + \
        workload + filename_subfix + ".csv"
    output_file2 = timeSpan + "_" + tier + \
        "_responsetime_wl" + workload + filename_subfix + ".csv"
    output_file3 = timeSpan + "_" + tier + \
        "_inout_wl" + workload + filename_subfix + ".csv"
...
def main():
    models = ["total", "StoriesOfTheDay", "Browse", "BrowseCategories", "BrowseStoriesInCategory", "OlderStories",
              "ViewStory", "ViewComment", "Search", "SearchInStories", "SearchInComments", "SearchInUsers"]
    models_title = ["total", "StoriesOfTheDay", "Browse", "BrowseCategories", "BrowseStoriesInCategory",
                    "OlderStories", "ViewStory", "ViewComment", "Search", "SearchInStories", "SearchInComments", "SearchInUsers"]
    for model in models:
        HTTP_input[model] = {}
        HTTP_output[model] = {}
        HTTP_multi[model] = {}
    ...
    for line in open(plist_file):
        try:
            ...
            # print parts
            stime_str = parts[0].replace(' ', '').replace(
                '[', '').replace('\'', '')
            etime_str = parts[1].replace(' ', '').replace(
                '[', '').replace('\'', '')
            reqType_str = parts[2].replace(
                ' ', '').replace('[', '').replace('\'', '')
            # print reqType_str
            ...
            if protocol == 'client' and reqRS < 1.0:
                ...
                addMulti2(stime, etime, stime_epoch, etime_epoch,
                          model, multi_count_in_sec, HTTP_multi)
                if reqRS < 0:
                    print(line)
                elif reqRS > 1.0:
                    addMulti2(stime, etime, stime_epoch, etime_epoch,
                              model, multi_count_in_sec, HTTP_multi_longReqs)
                    continue
                else:
                    incInOutRS(stime, reqRS, stime_epoch, etime_epoch,
                               model, multi_count_in_sec, HTTP_in, True)
                    incInOutRS(etime, reqRS, stime_epoch, etime_epoch,
                               model, multi_count_in_sec, HTTP_out_rs, True)
    ...
    for target_time in range(int(stime_epoch), int(etime_epoch) + 1):
        for ms in range(0, multi_count_in_sec):
            ...
            OUTFILE4.write(",%f,%f" % (HTTP_multi_longReqs["total"][ms_time], (
                HTTP_multi["total"][ms_time] - HTTP_multi_longReqs["total"][ms_time])))
            OUTFILE4.write("\n")
    OUTFILE4.close()
        ...
        for ms in range(0, multi_count_in_sec):
            ...
            for model in models:
                if (HTTP_in[model][ms_time][0] == 0):
                    ...
                    HTTP_in[model][ms_time][1] = HTTP_in[model][ms_time][1] / \
                        HTTP_in[model][ms_time][0]
                # ## the response time average using etime
                if (HTTP_out_rs[model][ms_time][0] == 0):
                    HTTP_out_rs[model][ms_time][1] = 0
                else:
                    HTTP_out_rs[model][ms_time][1] = HTTP_out_rs[model][ms_time][1] / \
                        HTTP_out_rs[model][ms_time][0]
...
def addMulti2(add_from, add_to, stime_epoch, etime_epoch, model,
    ...
    if (add_from2 <= add_to2):
        if (add_from2 - 1 >= stime_epoch * multi_count_in_sec):
            add_prev = math.ceil(add_from * multi_count_in_sec) - \
                (add_from * multi_count_in_sec)
            dic_multi['total'][add_from2 - 1] += add_prev
            dic_multi[model][add_from2 - 1] += add_prev
        add_post = (add_to * multi_count_in_sec) - \
            math.floor(add_to * multi_count_in_sec)
