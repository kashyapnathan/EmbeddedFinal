...
pwd = check_output('pwd').decode('utf-8')
hfile = pwd[:-1] + "/result.jtl"

results = subprocess.getstatusoutput(
...
runtime = subprocess.getstatusoutput("cat Experiments_timestamp.log")
    ...
    for line in f:
        ...
        if matches:
            ...
            if starttime >= runtime_start and starttime <= runtime_end:
                writer.writerow({'start_time': starttime, 'end_time': endtime,
                                'request_type': reqType, 'response_time': response})
