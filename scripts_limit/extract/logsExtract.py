...
print("{:13d} {:13d}".format(int(runtime_start), int(runtime_end)))
##########################################
                ...
                for line in f:
                    ...
                    if (line.split(' ')[4] == 'Conn'):
                        ...
                        fieldnames = ['starttime', 'endtime',
                                      'reqType', 'response', 'realType']
                        writer = csv.DictWriter(w, fieldnames=fieldnames)
                        if starttime >= runtime_start and starttime <= runtime_end:
                            # print("Hello")
                            writer.writerow({'starttime': starttime, 'endtime': endtime,
                                            'reqType': "StoriesOfTheDay", 'response': response, 'realType': typeStr})
            ...
            with open(hfile) as f, open(outFile, "w") as w:
                for line in f:
                    ...
                    fieldnames = ['starttime',
                                  'endtime', 'reqType', 'response']
                    writer = csv.DictWriter(w, fieldnames=fieldnames)
                    if starttime >= runtime_start and starttime <= runtime_end:
                        writer.writerow({'starttime': starttime, 'endtime': endtime,
                                        'reqType': "StoriesOfTheDay", 'response': response})
